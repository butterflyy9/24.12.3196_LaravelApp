    ...
    public function success($order_id)
    {
        ...
        try {
            ...
            if ($status) {
                ...
                // Jika API Midtrans mengonfirmasi bahwa transaksi telah
                // berhasil (settlement / capture)
                if (in_array($trx_status, ['settlement', 'capture'])) {

                    // Hanya lakukan update jika status di database lokal
                    // masih 'pending' (indikasi Webhook tidak masuk)
                    if (strtolower($transaction->status) === 'pending') {
                        ...
                        if ($transaction->event && $transaction->event->stock > 0) {
                            ...
                            try {
                                ...
                                \Log::error(
                                    'Gagal mengirim email E-Ticket secara manual (Bypass): ' . $e->getMessage()
                                );
                            }
                        }
                    }
            ...
            // Jika terjadi error dari API Midtrans (transaksi tidak valid),
